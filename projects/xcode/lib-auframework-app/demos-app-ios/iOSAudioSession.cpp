...
iOSAudioSession::iOSAudioSession(){
	 ...
	 */
	//
	if(AudioSessionInitialize(NULL, NULL, iOSAudioSession::escuchadorDeInterrupciones, this)!=0){ //run loop, run loop mode, metodo escuchador de interrupciones, datos que son pasados al metodo escuchador
		...
		AudioSessionAddPropertyListener (idCambioEscuchar, iOSAudioSession::escuchadorDeEventos, NULL);
...
bool iOSAudioSession::activarSesion(){
	bool exito = false;
...
bool iOSAudioSession::desactivarSesion(){
	bool exito = false;
	if(_sessionInterrumpida){
...
bool iOSAudioSession::sessionActivaYNoInterrumpida(){
	return _sessionActiva;
}

bool iOSAudioSession::sessionInterrumpida(){
	return _sessionInterrumpida;
}

bool iOSAudioSession::otroSonidoSeEstaReproduciendo(){
...
void iOSAudioSession::establecerContextoOpenAL(ALCcontext* contextoOpenAL){
...
bool iOSAudioSession::establecerCategoria(UInt32 kAudioSessionCategory_paramtro){
	/*
...
bool iOSAudioSession::sobreescribirSalida(bool haciaParlantes){
	bool exito = false;
...
bool iOSAudioSession::permitirCapturaDesdeBluetooth(bool permitirBluetooth){
	bool exito = false;
...
void iOSAudioSession::escuchadorDeInterrupciones(void* datosDeUsuario, UInt32 estadoDeInterrupcion){
	iOSAudioSession* sessionAudio = (iOSAudioSession*)datosDeUsuario;
...
void iOSAudioSession::escuchadorDeEventos(void* datosDeUsuario, AudioSessionPropertyID idPropiedad, UInt32 tamValorPropiedad, const void* valorPropiedad){
	/*CAMBIOS ESCUCHABLES
